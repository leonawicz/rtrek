...
There are eleven Memory Beta web portals available. To see them, call the main function for Memory Alpha access, `memory_beta`, and pass it `portals` as the API endpoint.
...
In contrast to the results of `memory_alpha("portals")`, notice that each `url` entry begins with `Category:` rather than `Portal`. The Memory Beta website does not explicitly offer "portals" like the Memory Alpha site. For consistency from the perspective of `rtrek`, several high-level categories features on the main page of Memory Beta are treated as portals.
...
One exception to the between-portal consistency is with the final portal in the above list, `timeline`. This "portal" option has been specially created from the `rtrek` side for pulling Memory Beta timeline data, which on the Memory Beta side is not as uniformly arranged as `rtrek` makes it appear.
