...
Currently, `memory_alpha` is available in `rtrek`. This is the main function associated with accessing data from Memory Alpha. For details and examples on this and its companion functions, see the Memory Alpha [vignette](https://leonawicz.github.io/rtrek/articles/memory-alpha.html). Functions for Memory Beta are not yet available but will be in the next package version.
